...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # [x] TODO: flatten the lists of arrays into single arrays, so that the rest of the code can be written in a vectorized
        # way. obs, actions, rewards, terminals, and q_values should all be arrays with a leading dimension of `batch_size`
        # beyond this point.
        obs = np.concatenate(obs) # concatenate along traj batch dimension
        ...
        # [x] TODO: update the PG actor/policy network once using the advantages
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            # [x] TODO: use the helper function self._discounted_return to calculate the Q-values
            q_values = [self._discounted_return(r) for r in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # [x] TODO: use the helper function self._discounted_reward_to_go to calculate the Q-values
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            # [x] TODO: if no baseline, then what are the advantages?
            advantages = q_values
        else:
            # [x] TODO: run the critic and use it as a baseline
            values = self.critic.forward(ptu.from_numpy(obs)).detach().cpu().numpy()
            assert values.shape == q_values.shape

            if self.gae_lambda is None:
                # [x] TODO: if using a baseline, but not GAE, what are the advantages?
        ...
        if self.normalize_advantages:
            # [?] what's the difference between different normalization 
